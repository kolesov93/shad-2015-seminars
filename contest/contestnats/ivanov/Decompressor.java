...
public class Decompressor {
    public static void main(String[] args) throws IOException {
        ...
        byte[] readed = Files.readAllBytes(Paths.get(inFile));
        String decodedText = Huffman.decode(readed);
        int k = decodedText.indexOf(' ');
        int var = Integer.parseInt(decodedText.substring(0, k));
        decodedText = decodedText.substring(k + 1);
        k = decodedText.indexOf(' ');
        BWT.our = Integer.parseInt(decodedText.substring(0, k));
        decodedText = decodedText.substring(k + 1);
        switch (var) {
            case 1:
                decodedText = bwtRleHuf(decodedText);
                break;
            case 2:
                decodedText = bwtMtf2RleHuf(decodedText);
                break;
            case 3:
                decodedText = rleBwtRleHuf(decodedText);
                break;
            case 4:
                decodedText = bwtMtf2Huf(decodedText);
                break;
        }
        PrintWriter out2 = new PrintWriter(outFile);
        out2.print(decodedText + '\n');
        out2.close();
    }

    static String bwtRleHuf(String source) {
        String decodedText = RLE.decode(source);
        return BWT.decode(decodedText);
    }

    static String bwtMtf2RleHuf(String source) {
        String decodedText = RLE.decode(source);
        decodedText = MTF2.decode(decodedText);
        return BWT.decode(decodedText);
    }

    static String rleBwtRleHuf(String source) {
        String decodedText = RLE.decode(source);
        decodedText = BWT.decode(decodedText);
        return RLE.decode(decodedText);
    }

    static String bwtMtf2Huf(String source) {
        String decodedText = MTF2.decode(source);
        decodedText = BWT.decode(decodedText);
        return RLE.decode(decodedText);
    }
